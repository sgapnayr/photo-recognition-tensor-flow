...
const BaseBodyPartLegend = ({ colorsOnScreen, scores, threshold, onThresholdChange }) => {
  ...
  const bodyPart = colorsOnScreen.map((color, idx) => {
    ...
    if (parts) {
      ...
      return (
        <div className="bg-white my-1 px-2 py-2 rounded-xl flex items-center justify-between w-[92.5%] max-w-[450px] whitespace-nowrap" key={idx}>
  ...
  return (
    <div className="flex flex-col text-center items-center w-full">
      <p className="opacity-20 text-2xl w-full">
        <span className="italic">Confidence:</span> ~{Confidence === 0 ? '<1' : Confidence?.toFixed(3)}%
      </p>
